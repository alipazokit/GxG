...
//vector <int> SNP_annot;
MatrixXdr annot_effect;
vector<double> beta_ef;
void read_annot_effect (string filename){
	...
	cout<<endl;
        ...
        while(std::getline (inp, line)){
		  ...
		  //annot_effect.resize(Nsnp,tokens.size());
			annot_effect=MatrixXdr::Zero(Nsnp,tokens.size());
                }
                for(int i = 0; i < tokens.size(); i++){
		      if (tokens[i]=="1"){
 		      		annot_effect(linenum,i)=1;
			}
                }
        linenum++;
       }
	  ...
	  if(Nsnp!=linenum){
          cout<<"Number of the rows in bim file and  EFFECT size annotation file does not match"<<endl;
        }
...
for(int i=0;i<num_snp;i++){
  ...
  //                                 val=0;
			  ...
			  if(annot_effect(i,2)==1)
				Enviro(j,gxg_index)=val;
...
void make_effect_size(double h2,string type,int num){
 ...
 if(type=="additive"){
	...
	cout<<type<<endl;
 	 ...
 	 cons_fact=(double)h2/annot_effect.col(0).sum();
	 A_effect=annot_effect.col(0)*cons_fact;
	for (int i=0;i<Nsnp;i++){
 ...
 if(type=="gxg"){
	 ...
	 GG_effect.resize(Nsnp,1);
	 GG_effect_multi.resize(Nsnp,num);
	 int num_gxg_pair=annot_effect.col(1).sum()*annot_effect.col(2).sum();
	 cout<<"# pairs of GxG"<<num_gxg_pair<<endl;
	 cons_fact=(double)h2/num_gxg_pair;
         GG_effect=annot_effect.col(1)*cons_fact;
 	for (int i=0;i<Nsnp;i++){
		boost::normal_distribution<> dist(0,sqrt(GG_effect(i,0)));
        	boost::variate_generator<boost::mt19937&, boost::normal_distribution<> > effect_size(seedr, dist);
		for (int j=0;j<num;j++){
			GG_effect_multi(i,j)=effect_size();
		}
	 }
 }	
...
void make_gxg_effect(double p_casual,double h2d,int num){
...
for (int jack_index=0;jack_index<Njack;jack_index++){
	...
	cout<<"Reading stream block"<<jack_index<<endl;
...
///////
	...
	MatrixXdr vec_eff;
	MatrixXdr vec_gxg_temp;
	for (int k=0;k<num_gxg_snp1;k++){
		vec_eff=GG_effect_multi.block(point_index,k,num_snp,1).transpose();
		vec_gxg_temp=compute_Xv(num_snp,vec_eff);
		vec_gxg_temp=vec_gxg_temp.array()*Enviro.col(k).array();
		simul_pheno+=vec_gxg_temp;
	}


	 vec_eff=A_effect_multi.block(point_index,0,num_snp,1).transpose();
...
string name=f3.str();
/////////////
string annot_adr=command_line_opts.Annot_PATH;
	read_annot_effect(annot_adr);
	cout<<"number of SNPs in G1 G2 G3:"<<endl;
	cout<<annot_effect.col(0).sum()<<" "<<annot_effect.col(1).sum()<<" "<<annot_effect.col(2).sum()<<endl;
	num_gxg_snp1=annot_effect.col(2).sum();
	Enviro.resize(Nindv,num_gxg_snp1);	
	read_bed(name,missing,Nsnp);
	for (int i=0;i<num_gxg_snp1;i++){
	    double mean_sel_snp=Enviro.col(i).sum()/Nindv;
	    double sd_sel_snp=sqrt((mean_sel_snp*(1-(0.5*mean_sel_snp))));
	    Enviro.col(i)=Enviro.col(i).array()-mean_sel_snp;
	    Enviro.col(i)=Enviro.col(i).array()/sd_sel_snp;
	}

double h2A=beta_ef[0];
double h2GG=beta_ef[1];
cout<<"h2A: "<<h2A<<endl;
cout<<"h2GxG: "<<h2GG<<endl;

Nz=1;	
num_simul=Nz;
k=num_simul;

	make_effect_size(h2A,"additive",1);
	cout<<" additive effect done"<<endl;

	make_effect_size(h2GG,"gxg",num_gxg_snp1);
	  cout<<"gxg effect done"<<endl;
...
process(name,1);
 ...
 boost::mt19937 seedr;
boost::normal_distribution<> dist(0,sqrt(1-h2A-h2GG));
