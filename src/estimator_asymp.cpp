...
for (int i=0;i<Nenv;i++){
  if(remove_self_inter==true && snp_in_annot==true )
                len.push_back(selected_snps-1); /// GxG ; remove selected snp
        else
                 len.push_back(selected_snps);
}
///


XXz=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin)*Nz);

if(both_side_cov==true){
UXXz=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin)*Nz);
XXUz=MatrixXdr::Zero(Nindv,(Nbin+nongen_Nbin)*Nz);
//Xz=MatrixXdr::Zero(Nindv,Nbin*(Njack+1)*Nz);
}
yXXy=MatrixXdr::Zero(Nbin+nongen_Nbin,1);
